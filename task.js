window.onload = function() {
    ...
    .then(( items ) => {
        ...
        contents.forEach((content)=>{
            ...
            let formatted_date = datetime.getFullYear() + "/" + ('0'+(datetime.getMonth() + 1)).slice(-2) + "/" + ('0'+datetime.getDate()).slice(-2) + " " + ('0'+datetime.getHours()).slice(-2) + ":" + ('0'+datetime.getMinutes()).slice(-2) + ":" + ('0'+datetime.getSeconds()).slice(-2);
            clone_element.getElementsByClassName("tasklist-course break course")[0].textContent = content["コース名"]
            clone_element.getElementsByClassName('tasklist-mobile-width-deadline deadline')[0].textContent = formatted_date
            var tasklist = clone_element.getElementsByClassName("tasklist-contents answer-test")
            console.log(content["リンク"])
